...
const AddCaregory = require('../model/AddCategory');
class CategoryController{
    ...
    try {
      const body = req.body;
      const reg = new AddCaregory(body);
  ...
  async getDataId(req, res) {
    try {
      ...
      const response = await AddCaregory.findById(id);
      if (!response) {
  ...
  async Put(req, res) {
    try {
      ...
      const response = await AddCaregory.findByIdAndUpdate(id, updatedData, { new: true });
  ...
  async Delete(req, res) {
    try {
      const id = req.params.id;
      if (!id) return res.status(400).json({ msg: "ID missing." });
      const response = await AddCaregory.findByIdAndDelete(id);
