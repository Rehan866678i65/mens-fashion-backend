...
const TechnicalPerson = require('../model/TechnicalPerson');
class TechnicalPersonController{


     async insertData(req, res) {
    try {
      const body = req.body;
      const reg = new TechnicalPerson(body);
  ...
  async getData(req, res) {
    try {
      ...
      const response = await TechnicalPerson.find({userId:userId});
      res.status(200).json({
        List: response,
  ...
  async getDataId(req, res) {
    try {
      ...
      const response = await TechnicalPerson.findById(id);
      if (!response) {
  ...
  async Put(req, res) {
    try {
      ...
      const response = await TechnicalPerson.findByIdAndUpdate(id, updatedData, { new: true });
  ...
  async Delete(req, res) {
    try {
      const id = req.params.id;
      if (!id) return res.status(400).json({ msg: "ID missing." });
      const response = await TechnicalPerson.findByIdAndDelete(id);
   ...
   async TechnicalPersonNameid(req, res) {
      try {
        var obj = req.params.FirstName;
        TechnicalPerson.find({
          $and: [
            { FirstName: obj }
          ]
        })
...
module.exports = TechnicalPersonController;